  ...
  sync:
    ...
    steps:
      ...
      - name: Pull in the latest public content
        run: |
          git remote add public https://github.com/my-docs-public/docs.git
...
This workflow reads from a public repo, so no secrets are required. Commit and push it to `staging`:
...
name: Promote page to public docs
on:
  workflow_dispatch:
    inputs:
      path:
        description: "Path to the file, relative to content/"
        required: true
...
jobs:
  promote:
    ...
    steps:
      ...
      - name: Copy the approved page across
        run: |
          mkdir -p "public/content/$(dirname "${{ inputs.path }}")"
          cp "private/content/${{ inputs.path }}" "public/content/${{ inputs.path }}"
          sed -i '/^visibility: staging-only$/d' "public/content/${{ inputs.path }}"
      ...
      - name: Open a PR against the public repo
        ...
        run: |
          git checkout -b "promote/${{ inputs.path }}"
          git add "content/${{ inputs.path }}"
          git commit -m "Promote ${{ inputs.path }} from private staging"
          git push origin "promote/${{ inputs.path }}"
          gh pr create --title "Promote: ${{ inputs.path }}" \
            --body "copied from the private staging repo, merge when the feature ships" \
            --base master
```
...
<Step>
### Close the staging pull request

Once reviewers have approved, close the pull request on the staging repo without merging. The page does not land on `staging/master`. Rebase from `origin/master`, squash your commits, and run the promote workflow to ship it.
</Step>

<Step>
### Run the promote workflow

Go to the private repo on GitHub, open Actions, select Promote page to public docs, and click Run workflow. Enter the file path relative to `content/`, for example `new-feature/index.mdx`, and run it.
