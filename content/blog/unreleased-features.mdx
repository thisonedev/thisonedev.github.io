...
on:
  workflow_dispatch:
    inputs:
      paths:
        description: "Comma-separated paths relative to content/ (e.g. docs/new-feature.mdx,blog/new-post.mdx,assets/01.png)"
        required: true
...
jobs:
  promote:
    ...
    steps:
      ...
      - name: Copy approved pages across
        run: |
          IFS=',' read -ra FILES <<< "${{ inputs.paths }}"
          for path in "${FILES[@]}"; do
            path=$(echo "$path" | xargs)
            mkdir -p "public/content/$(dirname "$path")"
            cp "staging/content/$path" "public/content/$path"
          done
      ...
      - name: Open a PR against the public repo
        ...
        run: |
          ...
          COMMIT_MSG=$(git -C $GITHUB_WORKSPACE/staging log -1 --pretty=%s)
