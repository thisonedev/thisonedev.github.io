---
title: How to convert GitBook API into OpenAPI + Mintlify
description: "Migrating API docs from GitBook to Mintlify OpenAPI: the problems you will hit with GitBook syntax, single-URL APIs, and Mintlify's YAML strictness"
date: 2026-06-23
---
...
As a technical writer working in web3, I recently run into a specific situation: a project I've been working with had their API reference on GitBook. The docs were functional, but GitBook wasn't pulling its weight for an API-heavy reference: no native OpenAPI rendering, no try-it playground, a sidebar you have to build by hand.

Mintlify had been on my list to test for a while, and this felt like the right moment. It renders OpenAPI natively, generates a clean sidebar grouped by tags, and includes a try-it playground without any extra setup. I used Hyperliquid's perpetuals API as my test case since it's a reference I know well and it's representative of the kind of multi-endpoint, single-URL APIs common in the space.

I'm also a regular user of AI tools for this kind of work. The entire conversion here was done through Claude. After several trial and errors, I figured out the right instructions to get consistent output. That took more iteration than expected because GitBook, OpenAPI, and Mintlify each introduce their own specific constraints. This post is about those constraints and how to address them in your prompt.
...
GitBook treats API documentation as just another markdown page. You write it, you format it, you ship it. There's no schema validation, no structured sidebar, and no automatic generation of anything. The result looks fine but requires a lot of manual maintenance as endpoints change.
...
Hyperliquid's info endpoint is a single URL with a single HTTP method. All 16 perpetuals operations post to `https://api.hyperliquid.xyz/info` and use a `type` field in the request body to distinguish between them: `"meta"`, `"clearinghouseState"`, `"fundingHistory"`, and so on.

OpenAPI expects paths to be unique combinations of URL and HTTP method. `POST /info` is one slot. Trying to describe 16 operations under that one key produces an invalid schema.
...
**Singular — drops the second shape:**
...
The overall approach works well as an AI-assisted task. The model handles the structural translation reliably once the edge cases are spelled out. What it doesn't do on its own is catch Mintlify-specific rendering quirks, which is what most of the prompt instructions above are actually for.