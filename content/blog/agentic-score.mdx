...
description: "Pushing the barriers of the Mintlify Agent Score on a static Fumadocs site: the five problems I hit getting to 93, the deploys that broke it, and a step-by-step guide for replicating the setup without the mistakes"
date: 2026-06-28
---
...
I spent this Friday night with one goal in mind: get a Grade A on the Mintlify Agent Score for my portfolio. That is the score [AFDocs](https://www.mintlify.com/score) runs against any docs site to measure how well AI agents can actually use it.

The site in question is the Fumadocs portfolio you are reading this on, hosted at `thisonedev.github.io` with no server runtime, no edge functions, and no headless CMS behind it. I started at 73 and finished at 93 over the course of one Friday night. This post is the actual work, including the two changes that briefly pushed the score back down before I figured out why.

## Why a technical writer should care about the Agent Score

The framing that pushed me to take the score seriously comes from Mintlify themselves, and it is worth quoting because it captures the whole context in three sentences:
...
[AFDocs](https://www.mintlify.com/score) turns that intuition into a checklist. The Agent Score crawls your site the way an agent would, runs around thirty checks, and reports what is missing. Most of the checks map to specific files or metadata that either exist on the site or do not.
...
## Problem 3: the sitemap and `llms.txt` disagreed on which URLs mattered
...
Consistency is what matters: same URL format in `llms.txt` and the sitemap, and AFDocs treats them as one set of real URLs. The strictly correct format would be HTML in `<loc>` with Markdown alternates. The format that actually works is the one the rest of the system also uses.
...
Claude Code's web fetch pipeline truncates at roughly 100KB of text before the summarization model ever sees it. Most other agent platforms are similar. The agent docs spec is explicit on the threshold: under 50K is comfortable, 50K to 100K is risky, over 100K is a fail. At 741KB my agents were seeing roughly the first 13% of the file, which was a couple of headers, the early docs, and then nothing. The remaining 641KB, including the bulk of the corpus, was invisible.

A root `llms-full.txt` becomes a small index pointing at per-section files (`/llms-full.txt/case-studies.md`, `/llms-full.txt/chainlink.md`, and so on), each under 50K characters. An agent reads the index, picks the section it needs, and fetches only that. Coverage goes back to 100%, and the per-section files stay under the truncation line. Step 6 builds the splitter.
...
A static MCP manifest dropped the score too. AFDocs's MCP check wants to find an MCP server at an expected endpoint, which on the surface sounded like a job for a static JSON file declaring the server. I wrote a `.well-known/mcp.json` with the standard metadata and pointed it at a fictional endpoint. AFDocs did not change the score. I should have seen that coming. The check probes for actual JSON-RPC behavior (initialize, listTools, that kind of thing), not for a manifest that says a server exists somewhere. A JSON file declaring a server does not satisfy a probe that asks the server to do something.

## Why 93 is the ceiling on GitHub Pages

The remaining 7 points split between recoverable work and the static-host ceiling.

The recoverable work is more of the same: tightening the content start position on a couple of pages, plus reducing the gap between rendered Markdown and source HTML on the one page that still has a parity warning, plus splitting `llms-full.txt` into per-section files so the whole corpus is visible to agents. Small adjustments like that. Pushing these would probably take the score into the mid-90s.
...
The two practical ways out are a custom domain pointed at Cloudflare with a Worker that does Accept-header routing and exposes an MCP endpoint, or moving the deployment to Netlify or Cloudflare Pages where edge functions handle both. Either path is real infrastructure work, not a config change. I am leaving the score at 93 for now because the project is a portfolio site, and the right answer for a portfolio is to use the platform you are documenting, not to spin up a Worker so the docs site scores one point higher on a benchmark.
...
The check measures DOM position relative to the full HTML stream, not presence. Placing the directive as a direct child of `<body>` on day one saves a debugging session later when the check reports "warning" on a file that is sitting right there in the source.
</Step>
<Step>
...
The agent discovery files are where the easy points live, and they are an afternoon of work. `llms.txt` with one line per page, `llms-full.txt` dumped from the same source, plus `skill.md` describing the site's shape. The Markdown export route is the next hour. The sitemap is the hour after that. None of those changes risk anything, and together they should land a Fumadocs or Docusaurus site in the low 90s.
...
The one trap I did not see coming was the LLMS directive. Place it as a direct child of `<body>` on day one and you will save yourself an afternoon of debugging why a check is at "warning" when the file is sitting right there in the source. The check measures DOM position relative to the full HTML stream, not presence.

The rest is just the kind of plumbing that anyone who has shipped docs before can pick up.